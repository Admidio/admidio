...
class ModuleMenu
{
	 ...
	 */
	private function mkItem($id, $type, $link, $text, $icon)
	{
		...
		return array('id'=>$id, 'type'=>$type, 'link'=>$link, 'text'=>$text, 'icon'=>$icon, 'subitems'=>array());
	}
	 ...
	 *  @param $desc Optional description of the menu item
	 */
	public function addItem($id, $link, $text, $icon)
	{
		$this->items[$id] = $this->mkItem($id, 'link', $link, $text, $icon);
	}
	...
	public function show()
	{
		...
		if(count($this->items) > 0)
		{
			...
			foreach($this->items as $key => $menuEntry)
			{
				...
				elseif($menuEntry['type'] == 'category')
				{
					...
					elseif($menuEntry['admin'] == true)
					{
						// show link to category preferences
						$html .= '
						<li id="'.$menuEntry['id'].'"><span class="iconTextLink">
							<a href="'.$this->root_path.'/adm_program/administration/categories/categories.php?type='.$menuEntry['categoryType'].'"><img
								src="'. THEME_PATH. '/icons/application_double.png" alt="'.$gL10n->get('SYS_MAINTAIN_CATEGORIES').'" /></a>
							<a href="'.$this->root_path.'/adm_program/administration/categories/categories.php?type='.$menuEntry['categoryType'].'">'.$gL10n->get('SYS_MAINTAIN_CATEGORIES').'</a>
						</span></li>';
					}
				}
			}
