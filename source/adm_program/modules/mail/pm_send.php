...
if ($getPMId == 0)
{
	...
	$sql = 'INSERT INTO '. TBL_PM. " (pm_id1, pm_id2, pm_subject, pm_usrid1, pm_usrid2, pm_message, pm_timestamp, pm_user1read, pm_user2read) 
...
else
{
	$sql = 'SELECT * 
              FROM '. TBL_PM. '
			  WHERE pm_id1 = '.$getPMId;

    $result = $gDb->query($sql);
	$PMId2 = $gDb->num_rows($result);
	
	$sql = 'UPDATE '. TBL_PM. " SET  pm_user2read = '1', pm_timestamp = CURRENT_TIMESTAMP
            WHERE pm_id2 = 0 and pm_id1 = ".$getPMId." and pm_usrid1 = '".$gCurrentUser->getValue('usr_id')."'";
    $gDb->query($sql);
	
	$sql = 'UPDATE '. TBL_PM. " SET  pm_user1read = '1', pm_timestamp = CURRENT_TIMESTAMP
