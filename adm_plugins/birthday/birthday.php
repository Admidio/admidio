    ...
    if($plg_show_names_extern === 1 || $gValidLogin)
    {
        ...
        echo '<ul id="plgBirthdayNameList">';
            while($row = $birthdayStatement->fetch())
            {
                ...
                if($gValidLogin)
                {
                    $plg_show_name = '<a href="'. ADMIDIO_URL. '/adm_program/modules/profile/profile.php?user_id='. $row['usr_id']. '"
                    ...
                    if(strlen($row['email']) > 0 && $plg_show_email_extern < 2)
                    {
                        $plg_show_name = $plg_show_name.'
                            <a class="admidio-icon-link" href="'. ADMIDIO_URL. '/adm_program/modules/messages/messages_write.php?usr_id='. $row['usr_id']. '"><img
