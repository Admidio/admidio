 ...
 */
     ...
     */
    static public function getTableLabel($table = null): array|string  {
        /**
     ...
     * **Code example**
     * ```
     * $usr = Changelog::getObjectForTable('users');
     * $usr->readDataById(500);
     * ```
     */
    static public function getObjectForTable(string $module): Entity | null {
     ...
     * 
     * The type can later be fed to the Changelog::formatValue function for proper HTML formatting
     * of a field value.
     * 
     * @return array
     * 
     * **Code example**
     * ```
     * $fieldNames = Changelog::getFieldTranslations();
     ...
     * ```
     */
    static public function getFieldTranslations() {
        global $gL10n;
     ...
     * ```
     */
    static public function createLink(string $text, string $module, int|string $id, string $uuid = '') {
        $url = '';
        switch ($module) {
            ...
            case 'roles_rights_data':
                ...
                return self::createLink($text, $table, $id, $id); break;
            case 'role_dependencies':
                $url = SecurityUtils::encodeUrl(ADMIDIO_URL.FOLDER_MODULES.'/groups-roles/groups_roles_new.php', array('role_uuid' => $uuid)); break;
            case 'rooms':
     ...
     * **Code example**
     * ```
     * $output = Changelog::formatValue("http://www.admidio.org/", "URL"); // Returns a link to the URL
     * $output = Changelog::formatValue(1, 'USER');  // Returns a link to the administrator, text is the administrator's name
     * ```
     */
    static public function formatValue($value, $type, $entries = []) {
