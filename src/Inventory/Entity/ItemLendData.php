 ...
 */
class ItemLendData extends Entity
{
    /**
    ...
    public function __construct(Database $database, ?ItemsData $itemsData = null, int $id = 0)
    {
        ...
        $this->connectAdditionalTable(TBL_INVENTORY_FIELDS, 'inf_id', 'inl_inf_id');
        parent::__construct($database, TBL_INVENTORY_ITEM_LEND_DATA, 'inl', $id);
    }
     ...
     * 
     * The log entries are: record ID for inl_id, but uuid and link point to User id.
     ...
     */
    protected function logLendChange(?string $oldval = null, ?string $newval = null) : bool {
        ...
        $field = $this->getValue('inl_inf_id');
        $fieldName = 'inl_value';
    ...
    {
        $oldval = $this->columnsInfos['inl_value']['previousValue'];
        return $this->logLendChange($oldval, null);
    }
    ...
    {
        if ($logChanges['inl_value']) {
            return $this->logLendChange($logChanges['inl_value']['oldValue'], $logChanges['inl_value']['newValue']);
        } else {
