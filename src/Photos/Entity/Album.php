<?php
namespace Admidio\Photos\Entity;
...
class Album extends Entity
{
     ...
     * default table will be read and stored in the object. Only albums of the current organization will be read.
     * If the ablum belongs to another organization than an exception will be thrown.
    ...
    protected function readData(string $sqlWhereCondition, array $queryParams = array()): bool
    {
        if (parent::readData($sqlWhereCondition, $queryParams)) {
            // check if album belongs to this organization
            if ($this->getValue('pho_org_id') > 0 && $this->getValue('pho_org_id') !== $GLOBALS['gCurrentOrgId']) {
                throw new Exception('Album ' . $this->getValue('pho_uuid') . ' belongs to another organization.');
            }
            return true;
        }
    ...
    public function sendNotification(): bool
    {
        ...
        if ($gSettingsManager->getBool('system_notifications_new_entries')) {
            ...
            $message = $gL10n->get($messageTitleText, array($gCurrentOrganization->getValue('org_longname'))) . '<br /><br />'
                ...
                . $gL10n->get('SYS_URL') . ': ' . ADMIDIO_URL . FOLDER_MODULES . '/photos.php?photo_uuid=' . $this->getValue('pho_uuid') . '<br />';
