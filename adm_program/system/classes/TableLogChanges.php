...
/**
 * Handle logging of changes to various different objects (user profile data, user fields, memberships, but also configuration of fields) and manage it in the database table adm_log_changes
 *
 * The class search in the database table **adm_log_changes** for role memberships of
 * users. It has easy methods to start or stop a membership.
 *
 * **Code example**
 * ```
 * // start membership without read data before
 * $logentry = new TableLogChanges($gDb);
 * $logentry->startMembership($roleId, $userId);
 *
 * // read log data and then stop membership
 * $logentry = new TableLogChanges($gDb);
 * $logentry->readDataByColumns(array('mem_rol_id' => $roleId, 'mem_usr_id' => $userId));
 * $logentry->stopMembership();
 * ```          
 */


// TODO_RK: Use $gSettingsManager->getBool('profile_log_edit_fields')
class TableLogChanges extends TableAccess
{
    /**
