...
class HtmlNavbar
{
     ...
     *  @param $data An array with all data if the item. This will be @id, @url, @text and @icon.
     *  @return Returns the html for the menu entry
	 */
	...
	public function addItem($id, $url, $text, $icon, $orientation = 'left', $parentItem = 'navbar', $class = '')
	{
	    ...
	    if(strlen($url) > 1 && preg_match('/^http(s?):\/\//', $url) == 0)
	    {
			$url = $g_root_path. $url;
        }

		// add THEME_PATH to images unless the full URL is given
		if(strlen($icon) > 0 && preg_match('/^http(s?):\/\//', $icon) == 0)
		{
