...
if ($getMode === 'new' || $getMode === 'change')
{
    ...
    if (strlen($phoBegin) > 0)
    {
        ...
        if ($startDate === false)
        {
            $gMessage->show($gL10n->get('SYS_DATE_INVALID', $gL10n->get('SYS_START'), $gSettingsManager->getString('system_date')));
            // => EXIT
        }
        else
        {
    ...
    if (strlen($phoEnd) > 0)
    {
        ...
        if ($endDate === false)
        {
            $gMessage->show($gL10n->get('SYS_DATE_INVALID', $gL10n->get('SYS_END'), $gSettingsManager->getString('system_date')));
            // => EXIT
        }
        else
        {
    ...
    if ($getMode === 'new')
    {
        ...
        if ($error === null)
        {
            ...
            try
            {
                $message = $gL10n->get('PHO_EMAIL_NOTIFICATION_MESSAGE', $gCurrentOrganization->getValue('org_longname'), $_POST['pho_name'], $gCurrentUser->getValue('FIRST_NAME').' '.$gCurrentUser->getValue('LAST_NAME'), date($gSettingsManager->getString('system_date')));
    ...
    else
    {
        ...
        if ($albumPath !== ADMIDIO_PATH . FOLDER_DATA . '/photos/' . $phoBegin . '_' . $getPhotoId)
        {
            ...
            {
                $gMessage->setForwardUrl(ADMIDIO_URL.FOLDER_MODULES.'/photos/photos.php');
                $gMessage->show($gL10n->get('SYS_FOLDER_WRITE_ACCESS', $newFolder, '<a href="mailto:'.$gSettingsManager->getString('email_administrator').'">', '</a>'));
                // => EXIT
            }
        }
