    ...
    if(strlen($user->getValue('LAST_NAME')) > 0 && strlen($user->getValue('FIRST_NAME')) > 0)
    {
        ...
        $sql = 'SELECT MAX(usr_id) AS usr_id
                  ...
                  JOIN '. TBL_USER_DATA. ' last_name
                   ...
                   AND last_name.usd_value  = \''. $gDb->escape_string($user->getValue('LAST_NAME', 'database')). '\'
                  JOIN '. TBL_USER_DATA. ' first_name
                    ON first_name.usd_usr_id = usr_id
                   AND first_name.usd_usf_id = '.  $gProfileFields->getProperty('FIRST_NAME', 'usf_id'). '
                   AND first_name.usd_value  = \''. $gDb->escape_string($user->getValue('FIRST_NAME', 'database')). '\'
                 WHERE usr_valid = 1 ';
