    ...
    <div class="formBody">
        ...
        if($date->getValue('dat_usr_id_create') > 0)
        {
            ...
            echo '<div class="editInformation">';
                ...
                echo $g_l10n->get('SYS_PHR_CREATED_BY', $user_create->getValue('FIRST_NAME'). ' '. $user_create->getValue('SURNAME'), $date->getValue('dat_timestamp_create'));

                if($date->getValue('dat_usr_id_change') > 0)
                {
                    $user_change = new User($g_db, $date->getValue('dat_usr_id_change'));
                    echo '<br />'.$g_l10n->get('SYS_PHR_LAST_EDITED_BY', $user_change->getValue('FIRST_NAME'). ' '. $user_change->getValue('SURNAME'), $date->getValue('dat_timestamp_change'));
                }
            echo '</div>';
        }
