...
if($getMode === 1 || $getMode === 5)  // Create a new event or edit an existing event
{
    ...
    if(isset($_POST['dat_all_day']))
    {
        ...
        $_POST['date_from_time']        = $midnightDateTime->format($gSettingsManager->get('system_time'));
        $_POST['date_to_time']          = $midnightDateTime->format($gSettingsManager->get('system_time'));
        $_POST['date_deadline_time']    = $midnightDateTime->format($gSettingsManager->get('system_time'));
    ...
    $startDateTime = \DateTime::createFromFormat($gSettingsManager->get('system_date').' '.$gSettingsManager->get('system_time'), $_POST['date_from'].' '.$_POST['date_from_time']);
    if(!$startDateTime)
    {
        // Error: now check if date format or time format was wrong and show message
        $startDateTime = \DateTime::createFromFormat($gSettingsManager->get('system_date'), $_POST['date_from']);

        if(!$startDateTime)
        {
            $gMessage->show($gL10n->get('SYS_DATE_INVALID', $gL10n->get('SYS_START'), $gSettingsManager->get('system_date')));
            // => EXIT
        }
        else
        {
            $gMessage->show($gL10n->get('SYS_TIME_INVALID', $gL10n->get('SYS_TIME').' '.$gL10n->get('SYS_START'), $gSettingsManager->get('system_time')));
            // => EXIT
        }
    }
    else
    ...
    $endDateTime = \DateTime::createFromFormat($gSettingsManager->get('system_date').' '.$gSettingsManager->get('system_time'), $_POST['date_to'].' '.$_POST['date_to_time']);

    if(!$endDateTime)
    {
        // Error: now check if date format or time format was wrong and show message
        $endDateTime = \DateTime::createFromFormat($gSettingsManager->get('system_date'), $_POST['date_to']);

        if(!$endDateTime)
        {
            $gMessage->show($gL10n->get('SYS_DATE_INVALID', $gL10n->get('SYS_END'), $gSettingsManager->get('system_date')));
            // => EXIT
        }
        else
        {
            $gMessage->show($gL10n->get('SYS_TIME_INVALID', $gL10n->get('SYS_TIME').' '.$gL10n->get('SYS_END'), $gSettingsManager->get('system_time')));
            // => EXIT
        }
    }
    else
    ...
    if($_POST['date_registration_possible'] == 1)
    {
        ...
        $deadlineDateTime = \DateTime::createFromFormat($gSettingsManager->get('system_date').' '.$gSettingsManager->get('system_time'), $_POST['date_deadline'].' '.$_POST['date_deadline_time']);
        if(!$deadlineDateTime)
        {
            $deadlineDateTime = \DateTime::createFromFormat($gSettingsManager->get('system_date'), $_POST['date_deadline']);
        }
    ...
    if($gSettingsManager->get('dates_show_rooms') == 1)
    {
    ...
    if($returnCode === true && $gSettingsManager->get('enable_email_notification') == 1)
    {
        ...
        try
        {
            ...
            if($getMode === 1)
            {
                $message = $gL10n->get('DAT_EMAIL_NOTIFICATION_MESSAGE_PART1', $gCurrentOrganization->getValue('org_longname'), $_POST['dat_headline'], $datum.' ('.$zeit.')', $calendar)
                          ...
                          .$gL10n->get('DAT_EMAIL_NOTIFICATION_MESSAGE_PART3', date($gSettingsManager->get('system_date')));
            ...
            else
            {
                $message = $gL10n->get('DAT_EMAIL_NOTIFICATION_CHANGE_MESSAGE_PART1', $gCurrentOrganization->getValue('org_longname'), $_POST['dat_headline'], $datum.' ('.$zeit.')', $calendar)
                          ...
                          .$gL10n->get('DAT_EMAIL_NOTIFICATION_CHANGE_MESSAGE_PART3', date($gSettingsManager->get('system_date')));
        ...
        if ($participationPossible)
        {
            switch ($getMode)
            {
                ...
                case 4:  // User cancel the event
                    if (!$gSettingsManager->get('dates_save_all_confirmations'))
                    {
                        // Delete entry
