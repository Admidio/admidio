...
{
    $sql_similar_name =
    '(  (   SUBSTRING(SOUNDEX(last_name.usd_value),  1, 4) LIKE SUBSTRING(SOUNDEX(\''. $gDb->escape_string($new_user->getValue('LAST_NAME', 'database')).'\'), 1, 4)
        AND SUBSTRING(SOUNDEX(first_name.usd_value), 1, 4) LIKE SUBSTRING(SOUNDEX(\''. $gDb->escape_string($new_user->getValue('FIRST_NAME', 'database')).'\'), 1, 4) )
     OR (   SUBSTRING(SOUNDEX(last_name.usd_value),  1, 4) LIKE SUBSTRING(SOUNDEX(\''. $gDb->escape_string($new_user->getValue('FIRST_NAME', 'database')).'\'), 1, 4)
        AND SUBSTRING(SOUNDEX(first_name.usd_value), 1, 4) LIKE SUBSTRING(SOUNDEX(\''. $gDb->escape_string($new_user->getValue('LAST_NAME', 'database')).'\'), 1, 4) ) )';
}
else
{
    $sql_similar_name =
    '(  (   last_name.usd_value  LIKE \''. $gDb->escape_string($new_user->getValue('LAST_NAME', 'database')).'\'
        AND first_name.usd_value LIKE \''. $gDb->escape_string($new_user->getValue('FIRST_NAME', 'database')).'\')
     OR (   last_name.usd_value  LIKE \''. $gDb->escape_string($new_user->getValue('FIRST_NAME', 'database')).'\'
        AND first_name.usd_value LIKE \''. $gDb->escape_string($new_user->getValue('LAST_NAME', 'database')).'\') )';
}
