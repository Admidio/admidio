...
if ($datesResult['numResults'] > 0)
{
    ...
    foreach ($datesResult['recordset'] as $row)
    {
        ...
        // i-cal downloadlink
        $description .= '<br /><br /><a href="' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/events/events_function.php', array('dat_id' => $dateId, 'mode' => '6')).'">' . $gL10n->get('DAT_ADD_DATE_TO_CALENDAR') . '</a>';

        // add entry to RSS feed
        $rss->addItem(
            $title,
            $description,
            SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/events/events.php', array('id' => $dateId, 'view' => 'detail')),
            $row['create_name'],
